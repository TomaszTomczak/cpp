...
public:
    struct Coordinate
    {
        Coordinate(int _x, int _y, int s)
        {
            x = _x;
            y = _y;
            steps = s;
...
public:
    ...
    {
        coordinates.push_back(Coordinate(0,0,0));
        for(auto x: wirePath)
        {
             calculateWirePoint(x);
        }
    ...
    std::vector<Coordinate> getStepsToCrossPoints(std::vector<Coordinate>& coords)
    {
        ...
        for(auto x : coordinates)
        {
            for(auto c : coords)
            {
                ...
                {
                    steps.push_back(x);
                    //std::cout<<"steps to x: "<<c.x<<" y: "<<c.y<<" is: "<<x.steps<<std::endl;
                }
            }
    ...
    static std::vector<Coordinate> getCrossPointWithOtherWire(Wire& firstWire, Wire& secondWire)
    {
        std::vector<Coordinate> crossPoints;
        for(auto first: firstWire.coordinates)
        {
            for(auto second: secondWire.coordinates)
            {
                if(first == second)
                {
                    
                    crossPoints.push_back(second);
                }
            }
        }
...
private:
    ...
    int currentStep=0;
    Coordinate isWirePointExists(int _x, int _y)
    {
        ...
        return Coordinate(_x,_y,s);
    }
    void addCoords(char direction)
    {
        
        switch(direction)
        {
            case 'L':
               // coordinates.push_back(Coordinate(coordinates.back().x-1,coordinates.back().y, currentStep));
               coordinates.push_back(isWirePointExists(coordinates.back().x-1,coordinates.back().y));
                break;
            case 'D':
                coordinates.push_back(isWirePointExists(coordinates.back().x,coordinates.back().y-1));
                break;
            case 'R':
                coordinates.push_back(isWirePointExists(coordinates.back().x+1,coordinates.back().y));
                break;
       ...
       // std::cout<<"x :"<<coordinates.back().x<< "y: "<<coordinates.back().y<<std::endl;
    }
    void calculateWirePoint(std::string stringCoords)
    {
        char operation = stringCoords.at(0);
        stringCoords.erase(0,1);
        int address = std::stoi(stringCoords);
...
std::vector<int> prepareData(int noun, int verb)
{
    std::vector<int> data = {1,noun,verb,3,1,1,2,3,1,3,4,3,1,5,0,3,2,1,10,19,1,6,19,23,1,10,23,27,2,27,13,31,1,31,6,35,2,6,35,39,1,39,5,43,1,6,43,
        47,2,6,47,51,1,51,5,55,2,55,9,59,1,6,59,63,1,9,63,67,1,67,10,71,2,9,71,75,1,6,75,79,1,5,79,83,2,83,10,87,1,87,5,91,1,91,9,95,1,
        6,95,99,2,99,10,103,1,103,5,107,2,107,6,111,1,111,5,115,1,9,115,119,2,119,10,123,1,6,123,127,2,13,127,131,1,131,6,135,1,135,10,
        139,1,13,139,143,1,143,13,147,1,5,147,151,1,151,2,155,1,155,5,0,99,2,0,14,0};
    
    return data;
    
}
int calculateLowestDistance(std::vector<Wire::Coordinate> coordinates)
{
    int lowestValue = coordinates.front().x + coordinates.front().y;
    for(auto x : coordinates)
    {
        if(x.x + x.y <= lowestValue)
        {
            
            lowestValue = abs(x.x) + abs(x.y);
            //std::cout<<"distance for x: "<<x.x<<"and y: "<<x.y<<" is: "<<abs(x.x) + abs(x.y)<<std::endl;
        }
    }
    return lowestValue;
};
...
{
    for(int i=0; i<=oper.size()-4; i=i+4)
    {
        bool terminate = false;
        
        switch(oper[i])
        {
            case 1: oper[oper[i+3]] = oper[oper[i+1]]+oper[oper[i+2]];  break;
            case 2: oper[oper[i+3]] = oper[oper[i+1]]*oper[oper[i+2]]; break;
            case 99: terminate=true; break;
        }
        if(terminate)
        {
            break;
...
// days
void adventCodeDay2_2()
{
    for(int noun=0; noun<99; noun++)
    {
        for(int verb = 0; verb<99; verb++)
        {
            if(19690720 == calculateData(prepareData(noun, verb)))
            {
...
void adventCodeDay3_2()
{
    ...
    std::vector<std::string> demo_1_2 = {"R8","U5","L5","D3"};
    std::vector<std::string> demo_2_2 = {"U7","R6","D4","L4"};

    std::vector<std::string> demo_1_3 = {"R75","D30","R83","U83","L12","D49","R71","U7","L72"};
    std::vector<std::string> demo_2_3 = {"U62","R66","U55","R34","D71","R55","D58","R83"};
    //R8,U5,L5,D3
    // U7,R6,D4,L4
    Wire first(firstWireData);
    std::cout<<"-------"<<std::endl;
    ...
    auto crossSections = Wire::getCrossPointWithOtherWire(first,second);
    auto stepsFirst = first.getStepsToCrossPoints(crossSections);
    auto stepsSecond = second.getStepsToCrossPoints(crossSections);
    
    //std::cout<<"steps First size: "<<stepsFirst.size()<<std::endl;
    //std::cout<<"steps Second size: "<<stepsSecond.size()<<std::endl;

int lowest = -1; 
    for(auto fs : stepsFirst)
    {
        for(auto ss: stepsSecond)
        {
            if(fs == ss)
            {
                if(lowest==-1)// first loop
                {
                  lowest = fs.steps+ss.steps;
                }
                else
                {
                    if(fs.steps+ss.steps<lowest)
                    {
                        lowest = fs.steps+ss.steps;
                    }
                }
    ...
    std::cout<<"lowest distance in steps is: "<<lowest<<std::endl;
